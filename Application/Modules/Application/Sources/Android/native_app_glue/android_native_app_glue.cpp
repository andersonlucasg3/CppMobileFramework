...
// Prevent the linker from pulling init_have_lse_atomics (and its .init_array entry)
// out of libclang_rt.builtins.  The compiler-rt object that defines the static
// init_have_lse_atomics also defines __aarch64_have_lse_atomics.  Providing our own
// definition (which we intentionally leave zero = "no LSE") satisfies every reference
// at link time, so --gc-sections can discard the entire compiler-rt object.  This is
// necessary for emulators with ndk_translation whose ARM64→x86_64 binary translator
// may crash inside init_have_lse_atomics (SIGSEGV, null dereference).
extern "C" int __aarch64_have_lse_atomics = 0;
